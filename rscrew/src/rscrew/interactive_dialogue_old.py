...
class OperatorDialogue:
    ...
    def generate_next_question(self, user_request: str, execution_context: str, question_num: int, max_questions: int) -> Optional[str]:
        """Generate the next question based on request and previous answers"""
        request_lower = user_request.lower()
        previous_answers = " ".join([answer.lower() for _, answer in self.qa_history])
        
        # Question 1: Technical stack/language preference
        if question_num == 1:
            if any(tech in request_lower for tech in ['api', 'web', 'app', 'system', 'database', 'server']):
                if not any(lang in request_lower for lang in ['python', 'javascript', 'java', 'go', 'rust', 'php', 'ruby']):
                    return "What's your preferred programming language or current tech stack?"
            return "Is this for learning, a personal project, or production use?"
        
        # Question 2: Purpose/context
        if question_num == 2:
            if "purpose" not in previous_answers and not any(purpose in previous_answers for purpose in ['learn', 'production', 'work', 'personal']):
                return "Is this for learning, a personal project, or production use?"
            elif any(word in request_lower for word in ['create', 'build', 'implement', 'develop']):
                return "Are you looking for a simple learning example or a production-ready solution?"
            return "Do you want something robust from the start, or prefer to start simple and expand?"
        
        # Question 3: Complexity/scope
        if question_num == 3:
            if not any(level in previous_answers for level in ['simple', 'basic', 'advanced', 'production', 'enterprise', 'robust']):
                return "Are you looking for a simple learning example or a production-ready solution?"
            elif any(word in request_lower for word in ['user', 'auth', 'login', 'management']):
                return "Do you need authentication, user roles, or just basic CRUD operations?"
            return "Any specific requirements for data storage, authentication, or integrations?"
        
        # Question 4: Specific requirements
        if question_num == 4:
            if any(word in request_lower for word in ['user', 'auth', 'login']) and "auth" not in previous_answers:
                return "Do you need authentication, user roles, or just basic CRUD operations?"
            elif any(word in request_lower for word in ['api', 'database', 'data']) and "database" not in previous_answers:
                return "Any specific requirements for data storage, authentication, or integrations?"
            return "Are you starting from scratch or integrating with existing systems?"
        
        # Question 5: Integration context
        if question_num == 5:
            if "existing" not in previous_answers and "scratch" not in previous_answers:
                return "Are you starting from scratch or integrating with existing systems?"
            return "Any other specific requirements or constraints I should know about?"
        
        return None  # No more questions needed
    
    def has_sufficient_context(self) -> bool:
        """Check if we have gathered sufficient context to proceed"""
        ...
        has_purpose = any(purpose in all_answers for purpose in ['learn', 'production', 'work', 'personal', 'experiment'])
        has_complexity = any(level in all_answers for level in ['simple', 'basic', 'advanced', 'production', 'enterprise', 'robust', 'minimal'])
        has_tech_context = any(tech in all_answers for tech in ['python', 'javascript', 'java', 'go', 'rust', 'php', 'ruby', 'node', 'react', 'flask', 'django']) or len(self.qa_history) >= 3
        
        # If we have at least 3 answers and cover 2/3 key areas, we have sufficient context
        return len(self.qa_history) >= 3 and sum([has_purpose, has_complexity, has_tech_context]) >= 2
